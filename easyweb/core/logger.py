...
def get_file_handler(log_dir=None):
    ...
    file_name = f'easyweb_{timestamp}.log'
...
easyweb_logger = logging.getLogger('easyweb')
easyweb_logger.setLevel(logging.INFO)
easyweb_logger.addHandler(get_file_handler())
easyweb_logger.addHandler(get_console_handler())
easyweb_logger.addFilter(SensitiveDataFilter(easyweb_logger.name))
easyweb_logger.propagate = False
easyweb_logger.debug('Logging initialized')
easyweb_logger.debug('Logging to %s', os.path.join(os.getcwd(), 'logs', 'easyweb.log'))
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record):
        ...
        super().emit(record)
        self.stream.close()
        easyweb_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1

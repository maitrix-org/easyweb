...
from fast_web.core.config import config
...
def get_file_handler(log_dir=None):
    ...
    file_name = f'fast_web_{timestamp}.log'
...
fast_web_logger = logging.getLogger('fast_web')
fast_web_logger.setLevel(logging.INFO)
fast_web_logger.addHandler(get_file_handler())
fast_web_logger.addHandler(get_console_handler())
fast_web_logger.addFilter(SensitiveDataFilter(fast_web_logger.name))
fast_web_logger.propagate = False
fast_web_logger.debug('Logging initialized')
fast_web_logger.debug(
    'Logging to %s', os.path.join(os.getcwd(), 'logs', 'fast_web.log')
)
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record):
        ...
        super().emit(record)
        self.stream.close()
        fast_web_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1

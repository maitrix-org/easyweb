...
class PromptedMemory(BaseMemory, AgentModule):
    ...
    def __call__(self, state, intent, llm_kwargs=None, **kwargs):
        ...
        user_prompt = self.prompt_template.format(
            memory=self.memory_content, state=state, intent=intent, **kwargs
        )
        llm_output = self.llm(
...
class StepPromptedMemory(StepKeyValueMemory):
    def __init__(
        self, identity, llm, prompt_template, keys, memory_key='memory_update'
    ):
        super().__init__(keys)
    ...
    def update(self, llm_kwargs=None, **kwargs):
        ...
        user_prompt = self.prompt_template.format(memory=self.get_value(), **kwargs)

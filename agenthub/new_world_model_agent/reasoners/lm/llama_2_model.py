...
from reasoners import GenerateOutput, LanguageModel
...
class Llama2Model(LanguageModel):
    ...
    def __init__(self, path, size, max_batch_size=1, max_seq_len=2048, **kwargs):
        ...
        self.model, self.tokenizer = self.build(
            os.path.join(path, f'llama-2-{size.lower()}'),
    ...
    def generate(
        ...
        **kwargs,
        ...
        if max_length is None:
            max_length = min(
                self.max_seq_len, self.model.params.max_seq_len - 1
            )  # use LLaMA's max length if not set
        if max_new_tokens is None:
            max_new_tokens = max_length  # set to a large number cannot be reached

        if not do_sample:
            if (
                temperature != 1.0 and self.local_rank == 0
            ):  # temperature is explicitly set with do_sample=False
                warnings.warn(
                    'temperature is set, but do_sample=False, so temperature will be ignored.'
                )  # if this feature for you is important, you can change the warning to error
            temperature = 0
        ...
        if eos_token_id_input is not None:
            ...
            for token in eos_token_id_input:
                if isinstance(token, str):
                    ...
                    token = tokenized[
                        -1
                    ]  # this feature can be changed to eos window if user really want a special word be as eos
        ...
        ), f'total batch size exceeds limit: {bsz} > {params.max_batch_size}'
        ...
        assert (
            max_prompt_size <= params.max_seq_len
        ), f'prompt length exceeds limit: {max_prompt_size} > {params.max_seq_len}'
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            if temperature > 0:
                ...
                )  # here is a modification on slice
        ...
        for candidate in candidates:
            ...
            for cand in candidate:
                ...
                cand_tokens[-1].append(
                    token[1] if token[0] == 29871 else token[0]
                )  ###?29871 I prefer to change this, same in llama1
    ...
    def get_loglikelihood(
        ...
        prompts_tokens = [
            self.tokenizer.encode(x, bos=True, eos=False) for x in contents
        ]

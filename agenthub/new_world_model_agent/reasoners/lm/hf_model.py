        ...
        **kwargs,
        ...
        if quantized == 'int8':
            ...
            try:
                ...
                print(
                    f'\033[31mError\033[0m: You need to install package awq to use {quantized=}. '
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_pth, trust_remote_code=True, lagacy=False
            )
            kwargs = {'torch_dtype': torch.float16, 'low_cpu_mem_usage': True}
            self.model = AutoModelForCausalLM.from_pretrained(
                model_pth, config=config, trust_remote_code=True, **kwargs
            )
            self.model.eval()
        ...
        if peft_pth is not None:
            self.model = PeftModel.from_pretrained(
                self.model, peft_pth, torch_dtype=torch.float16
            )

...
from llama1 import ModelArgs, Tokenizer, Transformer
...
class LlamaModel(LanguageModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if not do_sample:
            if (
                temperature != 0.0 and self.local_rank == 0
            ):  # temperature is explicitly set with do_sample=False #if do_sample=False, temperature set to not 0 have no effect
        ...
        if max_prompt_size > params.max_seq_len and self.local_rank == 0:
            warnings.warn(
                f'prompts exceed context length limit: {max_prompt_size} > {params.max_seq_len}'
            )
        ...
        for i, (t, input_t) in enumerate(zip(tokens.tolist(), prompt_tokens)):
            ...
            t = t[
                : len(prompt_tokens[i]) + max_length
            ]  # here is the difference, i think you want to set max_new_tokens
...
if __name__ == '__main__':
    ...
    print(
        model.get_next_token_logits(
            ['Smoking is harmful to health.'], candidates=[[' It']]
        )
    )
    print(
        model.get_next_token_logits(
            ['Smoking is harmful to health. It'], candidates=[[' is']]
        )
    )
    print(
        model.get_next_token_logits(
            ['Smoking is harmful to health. It is'], candidates=[[' the']]
        )
    )
    print(
        model.generate(

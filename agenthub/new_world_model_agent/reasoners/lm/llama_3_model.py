...
class Llama3Model(LanguageModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if not do_sample:
            if (
                temperature != 0.6 and self.local_rank == 0
        ...
        stop_tokens = torch.tensor(
            list(self.tokenizer.stop_tokens) + eos_token_id,
            dtype=torch.long,
            device='cuda',
        )
        ...
        for i, toks in enumerate(tokens.tolist()):
            ...
            if output_log_probs:
                probs = token_logprobs[i][
                    start : len(prompt_tokens[i]) + max_new_tokens
                ]
                print(len(probs))
...
if __name__ == '__main__':
    ...
    print(
        llama_model.generate(
            ['Do you love', 'The capital of France is'],
            eos_token_id=[13],
            output_log_probs=True,
        )
    )
    print(
        llama_model.get_next_token_logits(
            [
    ...
    print(
        llama_model.get_loglikelihood(
            'The capital of UK is',
            [
                'The capital of UK is Paris',
                'The capital of UK is London',
                'The capital of UK is Moscow',
            ],
        )
    )

...
class LlamaCppModel(LanguageModel):
    def __init__(self, path, n_ctx=2048, n_batch=512, n_thread=None):
        try:
            ...
            Llama = None
            raise ImportError(
                'You need to install llama-cpp-python if you want to use llama_cpp. '
    ...
    def generate(
        ...
        **kwargs,
        ...
        ), 'TODO: does not support multiple return sequences for llama.cpp now'
        if isinstance(eos_token_id, int):
            assert (
                False
            ), 'TODO: does not support *int* eos_token_id for llama.cpp now, use *str*'
        if isinstance(eos_token_id, list):
            assert not any(
                isinstance(token, int) for token in eos_token_id
            ), 'TODO: does not support *int* eos_token_id for llama.cpp now, use *str*'
        if not do_sample:
        ...
        for input in inputs:
            ...
            output = self.llama(
                ...
                top_p=top_p,
                stop=eos_token_id,
                logprobs=1 if output_log_probs else None,
    ...
    def get_loglikelihood(
        ...
        for content in contents:
            ...
            if any(p != c for p, c in zip(prefix_tokens, content_tokens)):
                warnings.warn(
                    f'prefix {repr(prefix)} does not match content {repr(content)}'
                )
            self.llama.reset()
            self.llama.eval(content_tokens)
            logits = self.llama.eval_logits
            log_probs = scipy.special.log_softmax(logits, axis=-1)
            content_log_probs = log_probs[
                np.arange(len(prefix_tokens), len(content_tokens)) - 1,
...
if __name__ == '__main__':
    ...
    print(
        model.get_next_token_logits(
            ['Hello'], candidates=[[',']], postprocess='log_softmax'
        )
    )
    print(
        model.get_next_token_logits(
            ['Hello,'], candidates=[[' I']], postprocess='log_softmax'
        )
    )
    print(
        model.get_next_token_logits(
            ['Hello, I'], candidates=[[' am']], postprocess='log_softmax'
        )
    )

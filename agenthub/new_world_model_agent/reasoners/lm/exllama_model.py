...
from .. import GenerateOutput, LanguageModel
...
class ExLlamaModel(LanguageModel):
    def __init__(
        ...
        try:
            ...
            sys.path.append(
                os.path.join(
                    os.path.dirname(reasoners.__file__), os.path.pardir, 'exllama'
                )
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if max_length is not None:
            warnings.warn(
                'max_length is not supported by ExLlamaModel for generation. Use max_new_tokens instead.'
            )
        if max_new_tokens is None:
            warnings.warn(
                f'max_new_tokens is not set, we will use the default value: {self.max_new_tokens}'
            )
        ...
        if eos_token_id_input is not None:
            ...
            for token in eos_token_id_input:
                if isinstance(token, str):
                    tokenized, mask = self.tokenizer.encode(
                        token, return_mask=True, add_bos=False, add_eos=False
                    )
        ...
        if num_return_sequences > 1:
            assert (
                len(inputs) == 1
            ), 'num_return_sequences > 1 is not supported for batched inputs'
        ...
        for start in range(0, len(inputs), self.max_batch_size):
            ...
            with torch.inference_mode():
                ...
                if self.log_time:
                    tqdm.write(
                        f'Time for generating {sum(num_new_tokens)} tokens: {round(t, 2)}s '
    ...
    def generate_simple(
        self, generator, prompt, max_new_tokens=128, eos_token_id=None, hide_input=False
    ):
        ...
        ids, mask = generator.tokenizer.encode(
            prompt, return_mask=True, max_seq_len=generator.model.config.max_seq_len
        )
    ...
    def get_next_token_logits(
        ...
        for candidate in candidates:
            ...
            for cand in candidate:
                ...
                cand_tokens[-1].append(
                    token[1].item() if len(token) > 1 else token[0].item()
                )
        ...
        tokens, mask = self.tokenizer.encode(
            prompt, return_mask=True, add_bos=True, add_eos=False
        )
        p_time = time.time()
        with torch.no_grad():
    ...
    def get_loglikelihood(
        ...
        prefix_tokens = self.tokenizer.encode(
            prefix, add_bos=True, add_eos=False
        ).squeeze(0)
        prompts_tokens, mask = self.tokenizer.encode(
            contents, return_mask=True, add_bos=True, add_eos=False
        )

...
from typing import Any, Callable, Generic, List, NamedTuple, Optional, Tuple, Union

import numpy as np

from .. import Action, SearchAlgorithm, SearchConfig, State, WorldModel
...
class BeamSearch(SearchAlgorithm, Generic[State, Action]):
    def __init__(
        ...
        reject_sample: Optional[bool] = None,
        reject_min_reward: Optional[float] = None,
        unbiased: Optional[bool] = None,
        reward_aggregator: Union[Callable[[List[Any]], float], str] = 'last',
        action_dedup: bool = False,
    ...
    def _initialize_reward_aggregator(self):
        ...
        if (
            self.reward_aggregator == 'cumulative'
            or self.reward_aggregator == 'accumulative'
        ):
            ...
            self.reward_aggregator, str
        ) and self.reward_aggregator.startswith('last'):
            ...
            if isinstance(self.reward_aggregator, str):
                raise NotImplementedError(
                    f'Reward aggregator {self.reward_aggregator} is not implemented.'
                )
    ...
    def _post_initialization(self):
        ...
        if self.temperature and self.temperature < 1e-4:
            ...
            warnings.warn(
                'Temperature is set to 0, sampling strategy is forced to be argmax.'
            )
        ...
        if not self.early_terminate:
            ...
            warnings.warn(
                'early_terminate is set to False, BeamSearch will return the beam instead of the best trace.'
            )
    ...
    def softmax(
        ...
        if unbiased and action_probs is not None:
            ...
            return [
                p / sum(adjusted_values) / max(1, len(adjusted_values)) for p in e_x
            ]
    ...
    def _sample(self, beam):
        if self.sampling_strategy == 'argmax':
            ...
            if self.unbiased:
                probs = BeamSearch.softmax(
                    rewards,
                    self.temperature,
                    self.unbiased,
                    action_probs=acc_action_probs,
                )

            else:
                probs = BeamSearch.softmax(
                    rewards, self.temperature, self.unbiased, action_probs=None
                )
            ...
            if self.reject_sample:
                ...
                while (
                    len(topk_beam_idx) < sample_size
                    and len(indexes)
                    and iterate_cnt < 100
                ):
                    iterate_cnt += 1
                    # sample an index
                    idx = random.choices(list(range(len(indexes))), weights=cur_probs)[
                        0
                    ]
                    idx = indexes[idx]

                    if random.uniform(0, 1) < cur_action_prob[idx] and rewards[
                        idx
                    ] > min(self.reject_min_reward, reward_upper_bound):
                        topk_beam_idx.append(idx)
                        indexes.remove(idx)

                        if self.unbiased:
                            cur_probs = BeamSearch.softmax(
                                [rewards[i] for i in indexes],
                                self.temperature,
                                self.unbiased,
                            ...
                            cur_probs = BeamSearch.softmax(
                                [rewards[i] for i in indexes], self.temperature
                            )

            else:
                topk_beam_idx = np.random.choice(
                    len(probs), size=sample_size, p=probs, replace=self.replace
                )
    ...
    def __call__(
        ...
        config: SearchConfig[State, Action, State],
    ):
        # reset id
                ...
                if self.early_terminate and world.is_terminal(state):
                    ...
                    for action in actions:
                        ...
                        if self.unbiased and self.sampling_strategy == 'stochastic':
                            ...
                            try:
                                fast_reward, fast_reward_aux = config.fast_reward(
                                    state, action
                                )
                                reward, reward_aux = config.reward(
                                    state, action, **aux, **fast_reward_aux
                                )
                                acc_action_prob = reward_aux['acc_action_prob']
                                cur_action_prob = reward_aux['cur_action_prob']
                            except KeyError:
                                raise ValueError(
                                    "If unbiased stochastic sampling is used, \
                        ...
                        if self.unbiased and self.sampling_strategy == 'stochastic':
                            new_beam.append(
                                (
                                    ...
                                    new_reward_list,
                                    new_reward,
                                    (acc_action_prob, cur_action_prob),
                                )
                            )
                        else:

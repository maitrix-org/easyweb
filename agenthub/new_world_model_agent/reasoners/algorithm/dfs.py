import itertools
from typing import Generic, List, NamedTuple, Optional, Tuple
...
class DFS(SearchAlgorithm, Generic[State, Action]):
    ...
    def __call__(self, world: WorldModel, config: SearchConfig):
        ...
        sorted_terminals = sorted(
            self.terminals, key=lambda x: sum(x.cum_rewards), reverse=True
        )
        result = DFSResult(
            terminal_state=sorted_terminals[0].state,
            cum_rewards=sorted_terminals[0].cum_rewards,
            tree_state=init_node,
            terminal_nodes=sorted_terminals,
        )
        return result
    ...
    def dfs(self, world: WorldModel, config: SearchConfig, cur_node: DFSNode):
        ...
        if self.prior:
            actions_with_prior = [
                (a, config.fast_reward(cur_state, a)) for a in new_actions
            ]
            new_actions = sorted(
                actions_with_prior, key=lambda x: x[1][0], reverse=True
            )
        else:
        ...
        for action in new_actions:
            ...
            if self.stat_cnt < self.total_states:
                ...
                new_node.reward, new_node.reward_details = config.reward(
                    cur_state, action, **aux, **fast_reward_details
                )
...
class CW_DFS(SearchAlgorithm, Generic[State, Action]):
    ...
    def __init__(
        self, total_states: int = 100, max_per_state: int = 3, depth: int = 10
    ):
        self.max_per_state = max_per_state
        self.depth = depth  # not used
        self.total_states = total_states
    ...
    def __call__(
        self, world: WorldModel, config: SearchConfig, best_state: bool = True
    ):
        init_state = world.init_state()
        self._reset()

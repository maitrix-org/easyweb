...
from .. import Action, Example, SearchAlgorithm, SearchConfig, State, Trace, WorldModel
...
class MCTS(SearchAlgorithm, Generic[State, Action, Example]):
    ...
    def _expand(self, node: MCTSNode):
        ...
        fast_rewards, fast_reward_details_list = self.search_config.fast_reward(
            node.state, actions
        )
        for action, fast_reward, fast_reward_details in zip(
            actions, fast_rewards, fast_reward_details_list
        ):
            child = MCTSNode(
                state=None,
                action=action,

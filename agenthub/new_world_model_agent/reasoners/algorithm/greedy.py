...
class GreedySearch(SearchAlgorithm, Generic[State, Action]):
    def __init__(
        ...
        temperature_decay: Optional[
            float
        ] = None,  # temperature decay, default to no decay
        reject_sample: Optional[
            bool
        ] = None,  # whether to reject the samples with reward less than the reject_min_reward
        reject_min_reward: Optional[
            float
        ] = None,  # the minimum reward to reject the sample
        unbiased: Optional[bool] = None,  # whether to use unbiased sampling
        reward_aggregator: Union[
            Callable[[List[Any]], float], str
        ] = 'last',  # how to aggregate the reward list
    ...
    def _post_initialization(self):
        ...
        if not self.early_terminate:
            ...
            warnings.warn(
                'early_terminate is set to False, GreedySearch will return the beam instead of the best trace.'
            )

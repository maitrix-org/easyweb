...
from .reasoner_connection import WebSearchConfig, WebWorldModel, apply_function
...
class NewWorldModelAgent(Agent):
    VERSION = '1.0'
    """
    ...
    def get_llm_output_new(
        ...
        if with_system_prompt:
            system_msg = prompt_factory.get_system_prompt(self.goal, self.action_space)
    ...
    def _encoder(self, current_obs, last_action, current_plan, state_history):
        obs_summary_prompt, answer_keys = prompt_factory.get_obs_summary_prompt(
            current_obs, last_action, current_plan
        )
        obs_summary = self.get_llm_output_new(obs_summary_prompt, answer_keys)[
            answer_keys[0]
        ]
        # logger.info(f'*Obs Summary*: {obs_summary}')

        state_prompt_answer_key_dict = prompt_factory.get_state_prompt(
            current_obs, obs_summary, last_action, state_history, current_plan
        )
        # state_dict = {'summary': obs_summary}
        # for prompt, keys in state_prompt_answer_key_dict.items():
        #     ans_dict = self.get_llm_output_new(prompt, keys)
        #     state_dict.update(ans_dict)

        with mp_dummy.Pool(processes=len(state_prompt_answer_key_dict)) as pool:
            arguments = [
                (prompt, keys) for prompt, keys in state_prompt_answer_key_dict.items()
            ]
            answer_dicts = pool.starmap(
                apply_function,
                [(self.get_llm_output_new, arg, {}) for arg in arguments],
            )
        ...
        for key in [
            ...
            if value != 'None':
                self.add_to_log(key, value)

        return state_dict

    def _effectuator(
        self, current_obs, current_state_dict, last_action, plan, state_history
    ):
        action_prompt, answer_keys = prompt_factory.get_action_prompt(
    ...
    def step(self, env_state: State) -> Action:
        ...
        if not self.is_planning and not self.finished_planning:
            ...
            self.add_to_log('obs', current_obs['axtree_txt'])
            self.last_action = (
        ...
        if not self.finished_planning:
            if replan:
                ...
                return FinishPlanningAction(current_strategy)
            else:
